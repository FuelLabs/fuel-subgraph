import { BigInt, Bytes, fuel } from "@graphprotocol/graph-ts"
...
function saveTransactions(
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    if (transaction.kind() == "mint") {
      ...
      tx_pointer.block_height = BigInt.fromU32(
        transaction.mint.tx_pointer.block_height
      )
      ...
      output_contract.input_index = BigInt.fromU32(
        transaction.mint.output_contract.input_index
      )
      output_contract.balance_root =
        transaction.mint.output_contract.balance_root
